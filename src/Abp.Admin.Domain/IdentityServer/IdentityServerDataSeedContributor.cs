...
namespace Abp.Admin.IdentityServer
{
    public class IdentityServerDataSeedContributor : IDataSeedContributor, ITransientDependency
    {
        ...
        private async Task CreateClientsAsync()
        {
            ...
            //Vue Client
            var vueClientId = configurationSection["Admin_Vue:ClientId"];
            if (!vueClientId.IsNullOrWhiteSpace())
            {
                var webClientRootUrl = configurationSection["Admin_Vue:RootUrl"]?.TrimEnd('/');

                await CreateClientAsync(
                    name: vueClientId,
                    scopes: commonScopes,
                    grantTypes: new[] { "password", "client_credentials", "authorization_code" },
                    secret: (configurationSection["Admin_Vue:ClientSecret"] ?? "1q2w3e*").Sha256(),
                    requireClientSecret: false,
                    redirectUri: webClientRootUrl,
                    postLogoutRedirectUri: webClientRootUrl,
                    corsOrigins: new[] { webClientRootUrl.RemovePostFix("/") }
